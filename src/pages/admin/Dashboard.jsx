...
import {
  ...
  UserPlus,
  CreditCard,
  Menu // ✅ Added: proper hamburger icon
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    if (mobileSidebarOpen) {
      ...
      document.body.style.position = "fixed"; // ✅ Extra iOS scroll lock
      ...
      setDashboardStats([
        {
          ...
          bg: "linear-gradient(135deg, #fff5eb 0%, #ffe8d1 100%)",
          color: "#e8590c"
        },
        {
          icon: DollarSign,
          label: "Revenue",
          value: formatCurrencyShort(d.revenue),
        ...
        },
        {
          ...
          bg: "linear-gradient(135deg, #e8f4fd 0%, #d0ebff 100%)",
          color: "#1971c2"
        },
        {
          icon: Users,
  ...
  const getActivityStyle = (type) => {
    switch (type) {
      case "order":
        return { Icon: ShoppingBag, color: "#f68b1e" };
      case "payment":
        return { Icon: CreditCard, color: "#099268" };
      case "user":
        return { Icon: UserPlus, color: "#0984e3" };
      case "stock":
        return { Icon: AlertTriangle, color: "#e8590c" };
      case "cart":
        return { Icon: ShoppingCart, color: "#6c5ce7" };
      case "cart_abandoned":
        return { Icon: AlertTriangle, color: "#e8590c" };
      default:
        return { Icon: Activity, color: "#868e96" };
    }
  };
  ...
  // ✅ Safe page setter that also closes sidebar
  ...
  return (
    <div className="dashboard-root">
      {/* ✅ Overlay with touch-action to prevent scroll passthrough on iOS */}
      {mobileSidebarOpen && (
        <div
      ...
      <aside
        className={`dashboard-sidebar ${sidebarCollapsed ? "dashboard-sidebar-collapsed" : ""} ${
          mobileSidebarOpen ? "dashboard-sidebar-mobile-open" : "dashboard-sidebar-mobile-closed"
        }`}
        // ✅ Prevent sidebar touch events from scrolling main content on iOS
        onTouchMove={(e) => {
          ...
          if ((atTop && e.touches[0].clientY > 0) || atBottom) {
            // Allow natural scroll within sidebar bounds
          }
        }}
      >
        <div className="dashboard-sidebar-header">
          {siteSettings?.logo ? (
            <img
              ...
              alt="Logo"
              className={`dashboard-logo-img ${sidebarCollapsed ? "dashboard-logo-img-collapsed" : ""}`}
            />
          ) : (
          ...
          {!sidebarCollapsed && (
            <div className="dashboard-sidebar-title-group">
              ...
              <span className="dashboard-sidebar-subtitle">
                {user?.role === "admin" ? "Administration" : "Sales Rep"}
              </span>
            </div>
          )}
        </div>
              ...
              >
                ...
                <div
                  className={`dashboard-nav-item-icon-wrap ${
                    activePage === item.key ? "dashboard-nav-item-icon-wrap-active" : ""
                  }`}
                >
                ...
                {!sidebarCollapsed && activePage === item.key && (
                  <ChevronRight width={16} className="dashboard-nav-item-arrow" />
                )}
              </button>
            );
          })}
        ...
        {/* ✅ Hide collapse button on mobile — no desktop sidebar on mobile */}
        <div className="dashboard-sidebar-footer dashboard-sidebar-footer--desktop">
          <button
            onClick={() => setSidebarCollapsed(!sidebarCollapsed)}
            className="dashboard-collapse-btn"
            title={sidebarCollapsed ? "Expand sidebar" : "Collapse sidebar"}
          >
            <div className={`dashboard-collapse-btn-icon ${sidebarCollapsed ? "rotated" : ""}`}>
        ...
        {/* ✅ Mobile close button */}
        <div className="dashboard-sidebar-footer dashboard-sidebar-footer--mobile">
          <button
            onClick={() => setMobileSidebarOpen(false)}
      ...
      <main className="dashboard-main">
        ...
        <header className="dashboard-header">
          ...
          <div className="dashboard-header-right">
            ...
            <button className="dashboard-header-notification" aria-label="Notifications">
              <Bell width={20} />
            ...
            <div className="dashboard-header-user">
              ...
              {/* ✅ Hide user text on mobile to save space */}
              <div className="dashboard-user-info">
                <span className="dashboard-user-name">{user?.name || "Admin"}</span>
                <span className="dashboard-user-role" style={{ textTransform: "capitalize" }}>
        ...
        <div className="dashboard-content">
            ...
            <div className="jm-dash-home">
              ...
              <div className="jm-dash-welcome">
                ...
                <button
                  ...
                  className="jm-dash-welcome__btn"
                >
                  ...
                  Array.from({ length: 4 }).map((_, idx) => (
                    ...
                    >
                      <div className="jm-stat-card__header">
                        <div
                          className="jm-stat-card__icon"
                          style={{ background: "#e9ecef", color: "#adb5bd" }}
                        >
                          <Activity width={22} />
                        </div>
                      </div>
                  ...
                  {quickActions.map((action, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className="jm-action-tile"
                        style={{ background: action.bg }}
                      >
                        <div className="jm-action-tile__bg-icon">
                  ...
                  {activityLoading ? (
                    Array.from({ length: 3 }).map((_, i) => (
                      <div key={i} className="jm-activity-item" style={{ opacity: 0.5 }}>
                        <div
                          className="jm-activity-item__icon"
          ...
          {activePage === "settings" && <SiteSettingsPage initialSettings={siteSettings} />}
        </div>
